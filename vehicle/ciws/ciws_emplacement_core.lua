...
function getModuleStatus(moduleBody, moduleJoint, moduleShapeTag, minVoxels, destroyedRatio)
	...
	if moduleShapeTag and moduleShapeTag ~= "" then
		...
		if shape ~= nil and IsHandleValid(shape) then
			local currentVoxels = GetShapeVoxelCount(shape)
			if cachedModuleInitialVoxels[cacheKey] == nil or currentVoxels > cachedModuleInitialVoxels[cacheKey] then
				cachedModuleInitialVoxels[cacheKey] = currentVoxels
			end

			local initialVoxels = cachedModuleInitialVoxels[cacheKey] or currentVoxels
			local ratioThreshold = destroyedRatio or 0.0
			local ratioDestroyed = ratioThreshold > 0.0 and initialVoxels > 0 and currentVoxels <= math.floor(initialVoxels * ratioThreshold)

			if currentVoxels >= minVoxels and not ratioDestroyed then
...
function getShootDir()
	...
	local useTrackedTarget = ciwsShouldUseTrackedTarget()

	if useTrackedTarget and ciwsMode.targetBody ~= 0 and IsHandleValid(ciwsMode.targetBody) then
...
function updateAiTracking(dt, trackingEnabled)
...
function serverGetModuleStatuses()
	return {
		gun = getModuleStatus(gunBody, pitchJoint, "ciws_barrel", 5),
		turret = getModuleStatus(turretBody, yawJoint, "ciws_turret", 10),
		radar = getModuleStatus(gunBody, 0, "ciws_radar", 10, 0.45),
		mount = getModuleStatus(gunBody, pitchJoint, "ciws_mount", 10),
	}
end

function serverTickHeat(dt, ignoreHeat)
	if ignoreHeat then
		barrelHeat = 0.0
		isOverheated = false
		return
...
function serverIsTrackingEnabled(radarStatus)
	return ciwsIsTrackingEnabled(radarStatus)
end

function serverTickIdle(dt, spin)
...
function serverUpdateAimMotors(ignoreDamagePenalty, statuses)
	local aimDir = TransformToParentVec(cameraTransform, Vec(0, 0, -1))
	aimDir = VecNormalize(aimDir)
	...
	if statuses.turret == "Destroyed" then
		desiredYawVelDeg = 0.0
	elseif statuses.turret == "Damaged" and not ignoreDamagePenalty then
	...
	if statuses.mount == "Destroyed" then
		desiredPitchVelDeg = 0.0
	elseif statuses.mount == "Damaged" and not ignoreDamagePenalty then
...
function serverTickFire(dt, spin, statuses, ignoreHeat, ignoreDamagePenalty, aiTrackingActive, yawError, pitchError)
	local canFire = statuses.gun ~= "Destroyed" and (ignoreHeat or not isOverheated)
	local requestedFire = ciwsResolveRequestedFire(dt, aiTrackingActive, yawError, pitchError)
	...
	if currentlyFiring and phalanxWeaponSpin.tryFire(spin, ciwsWeaponConfig) then
		...
		if statuses.gun == "Damaged" and not ignoreDamagePenalty then
...
function server.tick(dt)
	...
	local statuses = serverGetModuleStatuses()
	...
	serverTickHeat(dt, ignoreHeat)

	local trackingEnabled = serverIsTrackingEnabled(statuses.radar)
	local aiTrackingActive = false
	aiTrackingActive = updateAiTracking(dt, trackingEnabled)

	ciwsHandleServerRadarDestroyed(statuses.radar)

	if not serverControlActive and not autoFireEnabled and not aiTrackingActive then
		serverTickIdle(dt, spin)
		return
	end
...
function client.draw(dt)
	...
	local radarStatus = getModuleStatus(gunBody, 0, "ciws_radar", 10, 0.45)
	...
	UiPush()
		...
		local lockLabel = ciwsGetLockLabel()
		UiText(lockLabel .. (autoFireState.enabled and "ON" or "OFF"))
	UiPop()
...
function clientSyncAutoFireState()
...
function clientInitializeCameraState(pivot)
	...
	if not cameraState.initialized then
		...
		if dist > 0.1 then
			cameraState.distance = clamp(dist, cameraConfig.distMin, cameraConfig.distMax)
...
function clientUpdateCameraOrbit(dt)
...
function clientBuildCameraTransformFromPivot(pivot)
	local cfg = cameraConfig
...
function clientApplyMountedCamera()
	...
	if cameraAttachBody ~= 0 then
		...
		SetCameraTransform(cameraTransform, cameraConfig.fov)
	end
end

function clientGetModuleStatuses()
	return {
		gun = getModuleStatus(gunBody, pitchJoint, "ciws_barrel", 5),
		turret = getModuleStatus(turretBody, yawJoint, "ciws_turret", 10),
		radar = getModuleStatus(gunBody, 0, "ciws_radar", 10, 0.45),
...
function clientUpdateAutoLockAvailability(radarStatus)
	ciwsHandleClientRadarDestroyed(radarStatus)
end

function clientTickTurretRotationAudio(turretStatus)
...
function clientTickFiringEffects(dt, statuses, firing)
	if firing and statuses.gun ~= "Destroyed" and statuses.mount ~= "Destroyed" and not isOverheated then
		phalanxWeaponAudio.playSpin(audioState, ciwsWeaponConfig, cameraTransform.pos)
	end

	if barrelHeat > 0.01 and statuses.gun ~= "Destroyed" and gunBody ~= 0 then
		local smokeChance = barrelHeat * dt * 35.0
		if firing and not isOverheated then
			smokeChance = smokeChance * 1.5
		end
		...
		if math.random() < smokeChance then
			...
			ParticleAlpha(0.5 * barrelHeat, 0.0)
...
function client.tick(dt)
	...
	if ciwsCanToggleAutoMode() then
		ciwsHandleClientModeInput()
	end
	clientSyncAutoFireState()

	SetPlayerHidden()

	local pivot = clientGetCameraPivot()
	clientInitializeCameraState(pivot)
	clientUpdateCameraOrbit(dt)
	clientBuildCameraTransformFromPivot(pivot)
	clientApplyMountedCamera()

	local statuses = clientGetModuleStatuses()
	clientUpdateAutoLockAvailability(statuses.radar)
	clientTickTurretRotationAudio(statuses.turret)

	barrelHeat = GetFloat("vehicle."..vehicle..".barrelHeat")
	isOverheated = GetBool("vehicle."..vehicle..".isOverheated")

	local firing = ciwsGetClientRequestedFire()
	clientTickFiringEffects(dt, statuses, firing)
